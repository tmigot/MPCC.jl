...
"""
function JLag(nlp::BPMPCCModel, f::Function, c::Function, x::AbstractVector)
...
"""
function HLag(nlp::BPMPCCModel, f::Function, c::Function, x::AbstractVector)
...
function consG!(nlp::BPMPCCModel, x::AbstractVector, c::AbstractVector)
    ...
    c[1:nlp.meta.ncc] .=
        vcat(gxy - nlp.ll_lcon, -gxy + nlp.ll_ucon, y - nlp.ll_lvar, -y + nlp.ll_uvar)
    return c
end
...
function jac_nl_coord!(nlp::BPMPCCModel, x::AbstractVector, vals::AbstractVector)
    increment!(nlp, :neval_jac)
...
function jacG_coord!(nlp::BPMPCCModel, x::AbstractVector, vals::AbstractVector)
...
function jGprod!(nlp::BPMPCCModel, x::AbstractVector, v::AbstractVector, Jv::AbstractVector)
...
function jacH_coord!(nlp::BPMPCCModel, x::AbstractVector, vals::AbstractVector)
    m, n = nlp.meta.ncc, nlp.meta.nvar
...
function jHprod!(nlp::BPMPCCModel, x::AbstractVector, v::AbstractVector, Jv::AbstractVector)
...
function hess_coord!(
    ...
    temp(x) =
        nt ? obj_weight * nlp.f1(x) + dot(nlp.c(x), y[1:ncon-m]) :
        obj_weight * nlp.f1(x) + dot(cons_nl(nlp, x), y[1:ncon])
    ℓ(x) =
        llncon > 0 ?
        temp(x) + dot(nlp.g(x), y[ncon+1:ncon+llncon]) -
        dot(nlp.g(x), y[ncon+llncon+1:ncon+2*llncon]) : temp(x)
    Hx = ForwardDiff.hessian(ℓ, x)
    k = 1
    for j = 1:nlp.meta.nvar
        for i = j:nlp.meta.nvar
