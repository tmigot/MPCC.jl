...
function NLMPCC(mod::AbstractMPCCModel{T,S}; kwargs...) where {T,S}
    ...
    new_lcon = vcat(mod.meta.lcon, mod.cc_meta.lccG, mod.cc_meta.lccH, -Inf * ones(ncc))
    new_ucon = vcat(mod.meta.ucon, Inf * ones(2 * ncc), zeros(ncc))
    ...
    meta = NLPModelMeta(
        ...
        islp = false,
        name = "Nonlinear variant of $(mod.meta.name)",
    )
