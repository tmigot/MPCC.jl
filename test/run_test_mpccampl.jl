...
ux(i) = [100; 100][i]
lx(i) = [-100; -100][i]
JuMP.@variable(test1a, x[i = 1:2], upper_bound = ux(i), lower_bound = lx(i), start = 1.0)
JuMP.@NLobjective(test1a, Min, x[1] - x[2])
JuMP.@constraint(test1a, 1 - x[2] >= 0)

testa = MathOptNLPModel(test1a) #MathProgNLPModel(test1a)
mpcca = MPCCAmpl(testa, [0], 0)

@test mpcca.meta.ncc == 0
@test mpcca.meta.nvar == 2
@test mpcca.cvar == [0]
@test mpcca.meta.lvar == [-100.0; -100.0]
@test mpcca.meta.uvar == [100.0; 100.0]
...
@test length(hessH_coord(mpcca, mpcca.meta.x0, zeros(0))) ==
      mpcca.meta.nvar * (mpcca.meta.nvar + 1) / 2
@test length(hessG_coord(mpcca, mpcca.meta.x0, zeros(0))) ==
      mpcca.meta.nvar * (mpcca.meta.nvar + 1) / 2
